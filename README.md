...
**デモ: https://stockdesk-xi.vercel.app**

`admin@example.com` / `password123` でログインできます（権限違いのアカウントは下記）。
デモは誰でも操作できるため、データは実際に書き換わります。
...
> 無料枠のため、しばらくアクセスがないと DB が停止します。初回の表示だけ数秒かかることがあります。
