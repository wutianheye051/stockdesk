...
今月の受注金額、未出荷の件数、発注点を下回った商品、在庫金額。
「今なにをすべきか」が開いた瞬間に分かることを優先しています。

![ダッシュボード](docs/screenshots/01-dashboard.png)

### 商品・在庫の一覧

部分一致検索、カテゴリ・在庫状況・取扱状態での絞り込み、ソート、ページング。
発注点を下回った在庫数は赤で表示されます。CSV は**この絞り込み条件をそのまま引き継いで**出力されます。

![商品・在庫の一覧](docs/screenshots/02-products.png)

### 受注詳細

明細と、**この受注が動かした在庫の履歴**。右側の「ステータスを変更」には、
現在の状態から遷移できる操作だけが表示されます（確定済みなら「出荷済にする」と「キャンセルする」）。
確定すると在庫が引き当てられ、キャンセルすると戻ります。

![受注詳細](docs/screenshots/03-order-detail.png)

### 商品詳細 / 在庫操作

商品情報の編集と、入庫・出庫・棚卸調整。
在庫を動かすと必ず履歴に残り、`商品情報の更新`と`履歴の記録`は同一トランザクションで実行されます。

![商品詳細](docs/screenshots/04-product-detail.png)

### 閲覧専用アカウントから見た同じ画面

`viewer@example.com` でログインすると、同じ商品詳細でも**入力欄がすべて無効化され、
「在庫を動かす」フォームと保存ボタンが消えます**。
画面の出し分けは UX のためのもので、認可はサーバーアクション側でも検証しています。

![閲覧専用アカウントでの表示](docs/screenshots/05-viewer-readonly.png)
